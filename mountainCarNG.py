...
def plot_heights():
    ...
    plt.xlabel('Episode')
    plt.ylabel('Score')
    plt.plot(heights_t[:, 0].numpy())
    plt.plot(heights_t[:, 1].numpy())
    plt.plot(heights_t[:, 2].numpy())
...
env = MountainCar(speed=1e8, graphical_state=False, render=False, is_debug=True)
