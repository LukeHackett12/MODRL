...
class DQNAgent(object):
    def __init__(self, stateShape, actionSpace, numPicks, memorySize, sync=20, burnin=100, alpha=0.005, epsilon=1, epsilon_decay=0.01, epsilon_min=0.01, gamma=0.90):
    ...
    def trainDQN(self):
        if self.step <= self.numPicks and len(self.replayMemory) <= self.burnin:
            return 0
...
class DeepSeaTreasureBaselineDDQN(object):
    def __init__(self, episodes):
        ...
        self.agent = DQNAgent(stateShape=(
            2,), actionSpace=self.env.get_action_space(), numPicks=32, memorySize=2000)
    ...
    def episode(self):
        ...
        self.agent.terminal()
        print("now epsilon is {}, the reward is {} with loss {} in episode {}, step {}".format(
            self.agent.epsilon, rewardsSum, lossSum, self.current_episode, self.agent.step))
