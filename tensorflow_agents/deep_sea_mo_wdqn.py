...
class DQNAgent(object):
    def __init__(self, stateShape, actionSpace, numPicks, memorySize, numRewards, sync=100000, burnin=32, alpha=0.001, epsilon=1, epsilon_decay=0.01, epsilon_min=0.001, gamma=0.9):
    ...
    def trainDQN(self):
        ...
        for i, reward in enumerate(rewards):
            ...
            inverted_policy_mask = np.array([p for p in range(self.numPicks) if policies[p] != i and policies[p] != -1])
...
class DeepSeaWAgent(object):
    def __init__(self, episodes):
        ...
        self.env = DeepSeaTreasure(width=5, speed=15, graphical_state=False, render=True, is_debug=True)
